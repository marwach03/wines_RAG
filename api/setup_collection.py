...
def load_clients() -> None:
    """
    Initialise the Qdrant client and the BM25 sparse model once, at process
    startup, rather than on every request. The Ollama client is stateless
    (a thin HTTP wrapper) so it doesn't need explicit initialisation here.

    Failures here (Qdrant unreachable, BM25 model not downloadable yet,
    ...
    failing to start.
    """
    global qdrant_client, bm25_model, startup_error
    try:
